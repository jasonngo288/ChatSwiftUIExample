...
class ChatViewModel: ObservableObject {
    ...
    private func updateMessageImage() {
        ...
        UploadFileService.shared.uploadImage(withImage: image, imageName: "") { [weak self] imageId in
            if let _id = imageId {
                self?.service.sendMessage(_id, isImage: true, isVideo: false, isAudio: false)
            }
        }
    }
